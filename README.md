...
Start with [Introduction](src/introduction.md), the [table of contents](src/SUMMARY.md), and [Publication validation](src/open-decisions.md). The core, explicit Box/Vec APIs, conditional container traits, finite name/path rules, reference adjustments, program-end heap reclamation, reference allocator ABI, and execution-memory baseline have been specified. Integer-literal overflow, let/parameter collisions with visible unqualified const names, and equality between different source types are course UB. Supplied-parser and end-to-end runtime integration still need validation. Method lookup follows Rust within the supported subset. Assessment and scoring are separate course arrangements. The [backend draft](src/backend.md) is included in the book. The [C runtime example](src/runtime-example.c) implements I/O and the reference `__rx_alloc` wrapper.

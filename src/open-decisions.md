...
The allocator ABI, reference `Vec` growth policy, memory and stack settings, and cumulative heap budget are specified by the [backend resource guarantees](backend.md#resource-guarantees). Heap storage is reclaimed at program end; source ownership and reference validity follow the [heap rules](heap.md#program-end-reclamation).

...
An inherent impl defines associated items for a user-defined named-field struct. The implementing [Type] must resolve to that struct; a different kind of target is a compile error. Type parentheses retain their ordinary meaning, so `impl S { ... }` and `impl (S) { ... }` select the same struct.
